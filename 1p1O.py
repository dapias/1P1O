...
#      particula_i.v = (a_vieja*np.cos(oscilador_j.omega*delta_t + f_vieja))
   ...
   def colision_pared(self, particula):
       ...
       if particula.etiqueta == -1:
           particula.v = -particula.v #self.reservorio.velocidad(0)

       if particula.etiqueta == 1:
           particula.v = -particula.v #self.reservorio.velocidad(1)
...
#Los siguientes diccionarios permitirán hacer el "ploteo" después
        ...
        self.registro_velocidades2 = dict()
...
#        
    ...
    def run(self, imprimir = 0, flujos = 0, terminar = 0, cota = 2000,numero_puntos = 100):
        ...
        self.registro_velocidades2 = {"Particula" + str(i + 1) : [] for i in range(int(self.longpart))}
        ...
        # Si chocan la particula y el oscilador
                    ...
                    self.registro_velocidades2["Particula1"].append(siguiente_evento[0].v)
                    #self.registro_velocidades2["Particula1"].append(self.particulas[0].v)
                    
                    
#                    print siguiente_evento[0].v
                ...
                if imprimir == 1:
#                    print i + 1,  self.tiempo
                    print i, self.particulas[0].v
...
def plot_distribucion(sim):
    num_particulas = 1
#    num_osciladores = 1
    vx = [[] for _ in xrange(int(num_particulas))]
    
    num_eventos = sim.num_eventos_oscilador
...
def plot_ideal():
    ...
    for j in xrange(100000):
        k = np.random.random()
        vel.append(-(np.sqrt(-np.log(1.-k)*(T))))
    plt.figure()
    plt.hist(vel,50)
...
#    try:    
    sim.run(1,0,1, numero_puntos = num_puntos)
#    plot_datos(sim, num_total, frecuencia, 0)
    plot_distribucion(sim)
#    plot_ideal()
